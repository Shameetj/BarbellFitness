...
export default function WellnessScreen({ navigation }: Props) {
  ...
  const handleRequestPlan = async () => {
    ...
    try {
      ...
      const created = await createMembershipRequest(uid, 'Wellness');
      setPendingRequest(created);

      Alert.alert(
        'Request Submitted',
        'Your membership request for the Wellness Plan has been submitted. Please complete payment at the gym reception. Your membership will be activated after staff approval.',
        [
          {
            text: 'OK',
            onPress: () => {
